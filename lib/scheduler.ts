...
export async function listSchedules(params?: {
  ...
  try {
    ...
    if (!res) return { success: false, schedules: [], total: 0, error: 'No response from server' }
...
export async function getSchedulesForAgent(agentId: string): Promise<{
  ...
  try {
    ...
    const res = await fetchWrapper(`/api/scheduler?${qs}`)
    if (!res) return { success: false, schedules: [], webhooks: [], error: 'No response from server' }
    const data = await res.json()
...
export async function getScheduleLogs(
  ...
  try {
    ...
    const res = await fetchWrapper(`/api/scheduler?${qs}`)
    if (!res) return { success: false, executions: [], total: 0, error: 'No response from server' }
    const data = await res.json()
